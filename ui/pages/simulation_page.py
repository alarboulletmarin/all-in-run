...
def render_scenario_section(simulation_controller: SimulationController):
    ...
    """
    st.header(_("predefined_scenarios", "simulation_page"))
    ...
    if not st.session_state["scenarios"]:
        st.warning(_("no_scenarios_available", "simulation_page"))
        return
    ...
    for i, scenario in enumerate(st.session_state["scenarios"]):
        with st.expander(scenario["name"], expanded=False):
            ...
            st.markdown("**" + _("modified_parameters", "simulation_page") + ":**")

            for key, value in params.items():
                if key == "start_date" or key == "race_date":
                    st.write(f"- {_(key, 'simulation_page')}: {format_date(value)}")
                elif "pace" in key and isinstance(value, timedelta):
                    st.write(f"- {_(key, 'simulation_page')}: {format_pace(value)}")
                else:
                    st.write(f"- {_(key, 'simulation_page')}: {value}")

            # Bouton pour sélectionner ce scénario
            st.button(
                _("select_scenario", "simulation_page"),
                key=f"scenario_{i}",
...
def render_custom_simulation_form():
    ...
    """
    st.header(_("custom_simulation", "simulation_page"))
    ...
    if "current_plan" not in st.session_state or "user_data" not in st.session_state:
        st.warning(_("no_plan_available", "simulation_page"))
        return
    ...
    with st.form(key="simulation_form"):
        st.subheader(_("training_parameters", "simulation_page"))
        ...
        with col1:
            ...
            sessions = render_sessions_per_week_selector(
                label=_("sessions_per_week", "simulation_page"),
        ...
        with col2:
            ...
            min_volume, max_volume = render_volume_inputs(
                min_label=_("min_volume", "simulation_page"),
                max_label=_("max_volume", "simulation_page"),
        ...
        st.subheader(_("dates", "simulation_page"))
        ...
        with col1:
            ...
            start_date = render_date_selector(
                label=_("start_date", "simulation_page"),
                key="sim_start_date",
                default_value=st.session_state["simulation_params"].get(
        ...
        with col2:
            ...
            race_date = render_date_selector(
                label=_("race_date", "simulation_page"),
                key="sim_race_date",
                default_value=st.session_state["simulation_params"].get(
        ...
        st.subheader(_("paces", "simulation_page"))
        ...
        with col1:
            ...
            pace_5k = render_pace_input(
                label=_("pace_5k", "simulation_page"),
                key="sim_pace_5k",
            ...
            pace_half = render_pace_input(
                label=_("pace_half", "simulation_page"),
                key="sim_pace_half",
                default_value=format_pace(st.session_state["simulation_params"].get(
        ...
        with col2:
            ...
            pace_10k = render_pace_input(
                label=_("pace_10k", "simulation_page"),
                key="sim_pace_10k",
                default_value=format_pace(st.session_state["simulation_params"].get(
            ...
            pace_marathon = render_pace_input(
                label=_("pace_marathon", "simulation_page"),
        ...
        submit_button = st.form_submit_button(
            _("run_simulation", "simulation_page"),
...
def render_comparison_view():
    ...
    with col1:
        ...
        st.metric(
            label=_("total_volume", "simulation_page"),
    ...
    with col2:
        ...
        st.metric(
            label=_("total_duration", "simulation_page"),
    ...
    with col3:
        ...
        st.metric(
            label=_("total_weeks", "simulation_page"),
    ...
    tab1, tab2, tab3 = st.tabs([
        _("volume_comparison", "simulation_page"),
        _("intensity_comparison", "simulation_page"),
        _("sessions_comparison", "simulation_page")
    ])
    ...
    st.button(
        _("keep_simulation", "simulation_page"),
...
def render_simulation_page(
    ...
    if st.session_state.get("run_simulation", False):
        with st.spinner(_("running_simulation", "simulation_page")):
            ...
            st.success(_("simulation_success", "simulation_page"))
